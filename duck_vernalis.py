...
def SMD(system, prmtop, platform, platformProperties, temperature, positions, velocities, keyInteraction, spring_k, dist_in, dist_fin, SMD_num, save_step, move_force_step):
    ...
    for i in range(steps):    
        ...
        #reporter
        simulation.reporters.append(app.StateDataReporter("smd_"+str(temperature).split()[0].replace('.0','K')+"_"+str(SMD_num)+"_"+str(i)+".csv", move_force_step, step=True, time=True, totalEnergy=True, kineticEnergy=True, potentialEnergy=True,
                                                              temperature=True, density=True, progress=True, totalSteps=move_force_step, speed=True))
...
if keyInteraction == []:
    logging.info('Did not select key interaction')
    
logging.info('selected interaction: '+str(keyInteraction))
...
logging.info('Heating system under NVT...')
...
for temperature in (150.0, 200.0, 250.0, 300.0):
    ...
    logging.info('...to '+str(temperature).replace('.0', '')+' K...')
...
logging.info('Density correction under NPT...')  # Now that we are warm, let's allow the density to correct for 70 ps
...
logging.info('Wqb value '+str(Wqb_value))
if Wqb_value > Wqb_BLOCK_value:

    #SMD_325K_0
    logging.info('SMD_325K_0')
