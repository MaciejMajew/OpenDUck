...
def SMD(system, prmtop, platform, platformProperties, temperature, positions, velocities, PeriodicBoxVectors, keyInteraction, spring_k, dist_in, dist_fin, SMD_num, save_step, move_force_step):
    ...
    simulation.context.setPeriodicBoxVectors(PeriodicBoxVectors[0], PeriodicBoxVectors[1], PeriodicBoxVectors[2])
...
platformProperties0 = {}
platformProperties0['CudaPrecision'] = 'single'
platformProperties0['CudaDeviceIndex'] = '0'
...
simulation = app.Simulation(prmtop.topology, system, integrator, platform, platformProperties0)
...
print('Heating system under NVT to 300 K')
...
simulation.reporters.append(app.StateDataReporter("density.csv", 1000, step=True, time=True, totalEnergy=True, kineticEnergy=True, potentialEnergy=True,
                                                  temperature=True, density=True, progress=True, totalSteps=100000, speed=True))
...
positions = simulation.context.getState(getPositions=True).getPositions()
velocities = simulation.context.getState(getVelocities=True).getVelocities()
PeriodicBoxVectors = simulation.context.getState(enforcePeriodicBox=True).getPeriodicBoxVectors()
...
system_smd300 = prmtop.createSystem(nonbondedMethod=app.PME, nonbondedCutoff=9*u.angstrom, constraints=app.HBonds)
system_smd300.addForce(mm.MonteCarloBarostat(1.0*u.bar, 300*u.kelvin))
applyHarmonicPositionalRestraints(system_smd300, 1.0, inpcrd, Chunk_Heavy_Atoms)

SMD_num = 0
temperature = 300.0 * u.kelvin
SMD(system_smd300, prmtop, platform, platformProperties, temperature, positions, velocities, PeriodicBoxVectors, keyInteraction, spring_k, dist_in, dist_fin, SMD_num, save_step, move_force_step)
...
if Wqb_value > Wqb_BLOCK_value:
    ...
    # SMD_325K_0 #
    ##############
    system_smd325 = prmtop.createSystem(nonbondedMethod=app.PME, nonbondedCutoff=9*u.angstrom, constraints=app.HBonds)
    system_smd325.addForce(mm.MonteCarloBarostat(1.0*u.bar, 300*u.kelvin))
    applyHarmonicPositionalRestraints(system_smd325, 1.0, inpcrd, Chunk_Heavy_Atoms)
    
    print('SMD_325K_0')
    temperature = 325.0 * u.kelvin
    SMD(system_smd325, prmtop, platform, platformProperties, temperature, positions, velocities, PeriodicBoxVectors, keyInteraction, spring_k, dist_in, dist_fin, SMD_num, save_step, move_force_step)
    ...
    if Wqb_value > Wqb_BLOCK_value:
        ...
        for MD in range(num_MD):
            ...
            simulation.reporters.append(app.StateDataReporter("md_"+str(MD_num)+".csv", 1000, step=True, time=True, totalEnergy=True, kineticEnergy=True, potentialEnergy=True,
                                                  temperature=True, density=True, progress=True, totalSteps=250000, speed=True))
